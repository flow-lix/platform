...
public class ServiceRegistry extends AbstractZookeeperService {
    ...
    private void createNode(ZooKeeper zooKeeper, String data) {
        try {
            ...
            String path = zooKeeper.create(ZookeeperConstant.ZK_DATA_PATH, bytes, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
    ...
    private void addRootNode(ZooKeeper zooKeeper) {
        try {
            Stat stat = zooKeeper.exists(ZookeeperConstant.ZK_REGISTRY_PATH, false);
            if (stat == null) {
                zooKeeper.create(ZookeeperConstant.ZK_REGISTRY_PATH, new byte[]{}, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            }
        } catch (Exception e) {
            log.error("添加根节点失败!", e);
        }
